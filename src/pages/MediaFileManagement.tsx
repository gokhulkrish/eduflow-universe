...
import { getMedia, addMedia, deleteMedia, mediaFilesKey, MEDIA_TYPES, type MediaItem } from "@/lib/media";
...
export default function MediaFileManagement() {
  const [items, setItems] = useState<MediaItem[]>([]); const [loading, setLoading] = useState(false); const refresh = async () => { setLoading(true); setItems(await getMedia()); setLoading(false); };
  ...
  useRealtime("media_files", refresh);
    ...
    <div>
            ...
            <Card key={m.id} className="border-border/40">
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <Button variant="ghost" size="sm" className="rounded-lg h-7 w-7 text-destructive" onClick={async () => { await deleteMedia(m.id); await refresh(); toast.success("Deleted"); }}><Trash2 className="h-3 w-3" /></Button>
                </div>
              </CardContent>
            </Card>
          );
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button disabled={!name || !url} onClick={async () => { await addMedia({ name, type, url, size }); await refresh(); setOpen(false); toast.success("Added"); }}>Add</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
