  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search accreditation records..." className="flex-1 h-9 text-xs" />
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">Quality Cycle</Label><Input value={cycle} onChange={(e) => setCycle(e.target.value)} placeholder="e.g. 2024-25" /></div><div><Label className="text-xs">Framework</Label><Select value={frame} onValueChange={setFrame}><SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Select" /></SelectTrigger><SelectContent><SelectItem value="NAAC">NAAC</SelectItem><SelectItem value="NBA">NBA</SelectItem><SelectItem value="NIRF">NIRF</SelectItem><SelectItem value="AISHE">AISHE</SelectItem><SelectItem value="AQAR">AQAR</SelectItem><SelectItem value="Internal Audit">Internal Audit</SelectItem></SelectContent></Select></div></div>
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">Criterion / Metric</Label><Input value={criterion} onChange={(e) => setCriterion(e.target.value)} /></div><div><Label className="text-xs">Evidence Status</Label><Select value={evidence} onValueChange={setEvidence}><SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Select" /></SelectTrigger><SelectContent><SelectItem value="Pending">Pending</SelectItem><SelectItem value="Collected">Collected</SelectItem><SelectItem value="Verified">Verified</SelectItem><SelectItem value="Submitted">Submitted</SelectItem><SelectItem value="Accepted">Accepted</SelectItem></SelectContent></Select></div></div>
            <div><Label className="text-xs">Owner</Label><Input value={owner} onChange={(e) => setOwner(e.target.value)} /></div>
          </div>
