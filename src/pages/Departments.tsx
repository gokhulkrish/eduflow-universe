  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search departments..." className="flex-1 h-9 text-xs" />
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">Department Name</Label><Input value={name} onChange={(e) => setName(e.target.value)} /></div><div><Label className="text-xs">Department Code</Label><Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="e.g. CSE" /></div></div>
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">HOD / Coordinator</Label><Input value={hod} onChange={(e) => setHod(e.target.value)} /></div><div><Label className="text-xs">Program Level</Label><Select value={level} onValueChange={setLevel}><SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Select" /></SelectTrigger><SelectContent><SelectItem value="UG">UG</SelectItem><SelectItem value="PG">PG</SelectItem><SelectItem value="Research">Research</SelectItem><SelectItem value="Diploma">Diploma</SelectItem><SelectItem value="Certificate">Certificate</SelectItem></SelectContent></Select></div></div>
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">Sanctioned Intake</Label><Input type="number" value={intake} onChange={(e) => setIntake(e.target.value)} /></div><div><Label className="text-xs">NAAC / NBA Status</Label><Select value={naac} onValueChange={setNaac}><SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Select" /></SelectTrigger><SelectContent><SelectItem value="Not Applied">Not Applied</SelectItem><SelectItem value="Applied">Applied</SelectItem><SelectItem value="Accredited">Accredited</SelectItem><SelectItem value="Re-Accreditation Due">Re-Accreditation Due</SelectItem></SelectContent></Select></div></div>
          </div>
