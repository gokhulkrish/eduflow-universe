...
export default function PlacementCell() {
  ...
  const [jobs, setJobs] = useState(() => getJobs()); const [companies, setCompanies] = useState(() => getCompanies()); const [regs, setRegs] = useState(() => getRegistrations()); const [interviews, setInterviews] = useState(() => getInterviews());
  const analytics = getPlacementAnalytics();
  const refreshJobs = () => setJobs(getJobs()); const refreshCompanies = () => setCompanies(getCompanies()); const refreshRegs = () => setRegs(getRegistrations()); const refreshInterviews = () => setInterviews(getInterviews());
  ...
  const handleJobSave = () => {
    ...
    if (jEditId) { updateJob(jEditId, data); } else { createJob(data as any); }
    refreshJobs(); setJOpen(false); toast.success(jEditId ? "Updated" : "Created");
  };
  const handleStatus = (id: string, s: string) => { updateJob(id, { status: s }); refreshJobs(); toast.success(`Status: ${s}`); };
              ...
              <Card key={j.id} className="border-border/40">
                ...
                <CardContent className="space-y-1 text-xs">
                  ...
                  <div className="flex gap-2 pt-2 flex-wrap">
                    ...
                    <Button variant="outline" size="sm" className="rounded-lg h-7 text-[10px] text-destructive" onClick={() => { deleteJob(j.id); refreshJobs(); toast.success("Deleted"); }}><Trash2 className="h-3 w-3" /></Button>
                  </div>
                </CardContent>
              </Card>
            ))}
            ...
            <TableBody>
              ...
              {pag1.pageData.map((c) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell><Button variant="outline" size="sm" className="rounded-lg h-6 text-[9px] text-destructive" onClick={() => { deleteCompany(c.id); refreshCompanies(); toast.success("Deleted"); }}><Trash2 className="h-3 w-3" /></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
                  ...
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-1 flex-wrap">
                        {r.status === "registered" && <Button variant="outline" size="sm" className="rounded-lg h-6 text-[9px]" onClick={() => { updateRegistration(r.id, "shortlisted"); refreshRegs(); toast.success("Shortlisted"); }}>Shortlist</Button>}
                        {r.status === "shortlisted" && <Button variant="outline" size="sm" className="rounded-lg h-6 text-[9px]" onClick={() => { updateRegistration(r.id, "placed"); refreshRegs(); toast.success("Placed"); }}>Place</Button>}
