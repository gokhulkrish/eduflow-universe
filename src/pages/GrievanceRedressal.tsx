      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs" htmlFor="grievanceStudent">Student</Label><Input id="grievanceStudent" name="grievanceStudent" value={student} onChange={(e) => setStudent(e.target.value)} /></div><div><Label className="text-xs" htmlFor="grievanceCat">Category</Label><Select name="grievanceCat" value={cat} onValueChange={setCat}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["Academic", "Fee", "Facility", "Harassment", "Examination", "Administration", "Other"].map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div></div>
