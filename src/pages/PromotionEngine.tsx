...
import { promotionRulesKey, promotionRunsKey, getRules, createRule, updateRule, deleteRule, getPromotionRuns, runPromotion, getEligibleStudents, generateRollNumbers, reallocateSections, PROGRAMS, SECTIONS, nextProgram } from "@/lib/promotion";

export default function PromotionEngine() {
  const [tab, setTab] = useState("rules");
  const [rules, setRules] = useState(() => getRules());
  const [runs, setRuns] = useState(() => getPromotionRuns());
  ...
  const refreshRuns = () => setRuns(getPromotionRuns());
  ...
  const handleProgramChange = (g: string) => { setFromProgram(g); setToProgram(nextProgram(g)); };

  const openEdit = (r?: any) => {
    setEditId(r?.id ?? null); setName(r?.name ?? ""); setFromProgram(r?.from_grade ?? ""); setToProgram(r?.to_grade ?? ""); setFromSec(r?.from_section ?? ""); setToSec(r?.to_section ?? ""); setMinAtt(String(r?.min_attendance ?? 75)); setMinGpa(String(r?.min_gpa ?? 2.0)); setAuto(r?.auto_promote ?? false); setResetRoll(r?.reset_roll ?? false); setOpen(true);
  };

  const handleRun = (r: any) => {
    runPromotion(r, 120);
    refreshRuns();
    toast.success(`Promotion run: ${r.promoted || 0} promoted, ${r.failed || 0} failed`);
  };

  const handlePreview = (r: any) => {
    setPreview(getEligibleStudents(r));
    toast.success(`Preview: ${preview.filter((s) => s.eligible).length} eligible`);
  };

  const pag1 = usePagination({ data: preview, pageSize: 10 });
  const pag2 = usePagination({ data: runs, pageSize: 10 });

  useEffect(() => subscribeAppSync([promotionRulesKey, promotionRunsKey], () => {
    refreshRules();
    refreshRuns();
  }), []);
  ...
  return (
    <div>
      <PageHeader title="Cohort Progression Engine" subtitle="Rules, preview, roll reset & section reallocation" icon={<GraduationCap className="h-6 w-6" />} />
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="rules">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {rules.length === 0 && <Card className="col-span-full"><CardContent className="py-12 text-center text-sm text-muted-foreground">No promotion rules</CardContent></Card>}
            {rules.map((r) => (
              <Card key={r.id} className="border-border/40">
                ...
                <CardContent className="text-xs space-y-1">
                  <p><strong>Cohort:</strong> {r.from_grade}{r.from_section ? `-${r.from_section}` : ""} → {r.to_grade}{r.to_section ? `-${r.to_section}` : ""}</p>
                  <p className="text-muted-foreground">Min attendance: {r.min_attendance}% · Min GPA: {r.min_gpa}</p>
                  <p className="text-muted-foreground">Auto: {r.auto_promote ? "Yes" : "No"} · Reset roll: {r.reset_roll ? "Yes" : "No"}</p>
                  <div className="flex gap-2 pt-2 flex-wrap">
                    <Button variant="outline" size="sm" className="rounded-lg h-7 text-[10px]" onClick={() => openEdit(r)}>Edit</Button>
                    <Button variant="outline" size="sm" className="rounded-lg h-7 text-[10px]" onClick={() => { handlePreview(r); setTab("preview"); }}><Eye className="h-3 w-3 mr-1" />Preview</Button>
                    <Button variant="outline" size="sm" className="rounded-lg h-7 text-[10px]" onClick={() => handleRun(r)}><Play className="h-3 w-3 mr-1" />Run</Button>
                    <Button variant="outline" size="sm" className="rounded-lg h-7 text-[10px] text-destructive" onClick={() => { deleteRule(r.id); refreshRules(); toast.success("Deleted"); }}><Trash2 className="h-3 w-3" /></Button>
                  </div>
                </CardContent>
              </Card>
            ))}
          ...
          <Card>
            ...
            <CardContent>
              ...
              <TableHeader className="">
                <TableRow><TableHead className="text-xs">Student</TableHead><TableHead className="text-xs">Attendance</TableHead><TableHead className="text-xs">GPA</TableHead><TableHead className="text-xs">Eligible</TableHead></TableRow>
              </TableHeader>
                <TableBody>
                  {preview.length === 0 && <TableRow><TableCell colSpan={4} className="text-center text-xs text-muted-foreground py-8">Select a rule and click Preview</TableCell></TableRow>}
                  {pag1.pageData.map((s) => (
                    <TableRow key={s.id}>
              ...
              <div className="flex gap-2 mb-4">
                {SECTIONS.map((s) => (
                  <Button key={s} variant="outline" size="sm" className="rounded-lg h-7 text-[10px]" onClick={() => {
                    ...
                    const alloc = reallocateSections(dummy, SECTIONS);
                    setSectionAlloc(alloc);
                    toast.success(`${SECTIONS.length} sections populated`);
                  }}>Reallocate {s}</Button>
                ))}
              </div>
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs">From Cohort</Label><Select value={fromProgram} onValueChange={handleProgramChange}><SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger><SelectContent>{PROGRAMS.map((g) => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent></Select></div>
              <div><Label className="text-xs">To Cohort</Label><Select value={toProgram} onValueChange={setToProgram}><SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger><SelectContent>{PROGRAMS.map((g) => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent></Select></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs">From Section</Label><Select value={fromSec} onValueChange={setFromSec}><SelectTrigger><SelectValue placeholder="Any" /></SelectTrigger><SelectContent>{SECTIONS.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
              <div><Label className="text-xs">To Section</Label><Select value={toSec} onValueChange={setToSec}><SelectTrigger><SelectValue placeholder="Any" /></SelectTrigger><SelectContent>{SECTIONS.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
          ...
          <DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={() => { const payload: any = { name, from_grade: fromProgram, to_grade: toProgram, from_section: fromSec, to_section: toSec, min_attendance: Number(minAtt), min_gpa: Number(minGpa), auto_promote: auto, reset_roll: resetRoll, status: "active" }; if (editId) { updateRule(editId, payload); } else { createRule(payload); } refreshRules(); setOpen(false); toast.success(editId ? "Updated" : "Created"); }} disabled={!name || !fromProgram || !toProgram}>{editId ? "Update" : "Create"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
