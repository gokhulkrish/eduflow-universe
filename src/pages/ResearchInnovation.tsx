...
import { FlaskConical, Plus, Pencil, Trash2, Search, Download, Upload } from "lucide-react";
...
import { getResearch, createResearch, updateResearch, deleteResearch, Research } from "@/lib/research";

export default function ResearchInnovation() {
  const navigate = useNavigate();
  const [items, setItems] = useState<Research[]>([]); const [search, setSearch] = useState("");
  const refresh = useCallback(async () => { setItems(await getResearch()); }, []);
  useEffect(() => { refresh(); }, [refresh]);
  useRealtime("research_projects", refresh);
  const [open, setOpen] = useState(false); const [editId, setEditId] = useState<string | null>(null); const [title, setTitle] = useState(""); const [pi, setPi] = useState(""); const [agency, setAgency] = useState(""); const [amount, setAmount] = useState(""); const [stage, setStage] = useState(""); const [deleteId, setDeleteId] = useState<string | null>(null);
  const openAdd = () => { setEditId(null); setTitle(""); setPi(""); setAgency(""); setAmount(""); setStage(""); setOpen(true); };
  const openEdit = (r: Research) => { setEditId(r.id); setTitle(r.researchTitle); setPi(r.principalInvestigator); setAgency(r.fundingAgency); setAmount(r.grantAmount?.toString() ?? ""); setStage(r.researchStage); setOpen(true); };
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4">
        ...
        <Button size="sm" variant="outline" className="rounded-xl" onClick={() => navigate("/import?module=research")}><Upload className="h-4 w-4 mr-1" /> Import</Button>
        <Button size="sm" variant="outline" className="rounded-xl" onClick={() => exportToCsv(items, "research", [{key:"researchTitle",label:"Title"},{key:"principalInvestigator",label:"PI"},{key:"fundingAgency",label:"Funding Agency"},{key:"grantAmount",label:"Grant Amount"},{key:"researchStage",label:"Stage"}])}><Download className="h-4 w-4 mr-1" /> Export</Button>
        <Button size="sm" className="rounded-xl bg-gradient-primary shadow-glow" onClick={openAdd}><Plus className="h-4 w-4 mr-1" /> Add Project</Button>
      </div>
      <TablePagination {...pag} />
      <Table>
        ...
        <TableBody>
          {pag.pageData.map((r) => (
            <TableRow key={r.id}>
              ...
              <TableCell><div className="flex gap-1"><Button variant="outline" size="sm" className="rounded-lg h-6 text-[9px]" onClick={() => openEdit(r)}><Pencil className="h-3 w-3" /></Button><Button variant="outline" size="sm" className="rounded-lg h-6 text-[9px] text-destructive" onClick={() => setDeleteId(r.id)}><Trash2 className="h-3 w-3" /></Button></div></TableCell>
            </TableRow>
          ))}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Project</AlertDialogTitle><AlertDialogDescription>This will permanently remove this research project. Continue?</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel onClick={() => setDeleteId(null)}>Cancel</AlertDialogCancel><AlertDialogAction className="bg-destructive text-destructive-foreground" onClick={async () => { if (deleteId) { await deleteResearch(deleteId); refresh(); toast.success("Deleted"); } setDeleteId(null); }}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
      </AlertDialog>

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader><DialogTitle>{editId ? "Edit Research Project" : "Add Research Project"}</DialogTitle></DialogHeader>
