    ...
    <div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div><Label className="text-xs" htmlFor="bulkType">Type</Label><Select name="bulkType" value={bulkType} onValueChange={setBulkType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="student">Students</SelectItem><SelectItem value="staff">Staff</SelectItem></SelectContent></Select></div>
                <div><Label className="text-xs" htmlFor="bulkPrefix">ID Prefix</Label><Input id="bulkPrefix" name="bulkPrefix" value={bulkPrefix} onChange={(e) => setBulkPrefix(e.target.value)} /></div>
                <div><Label className="text-xs" htmlFor="bulkCount">Count</Label><Input id="bulkCount" name="bulkCount" type="number" value={bulkCount} onChange={(e) => setBulkCount(e.target.value)} /></div>
              </div>
              <div><Label className="text-xs" htmlFor="templateId">Template</Label><Select name="templateId" value={templateId} onValueChange={setTemplateId}><SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger><SelectContent>{templates.map((t) => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent></Select></div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs" htmlFor="idType">Type</Label><Select name="idType" value={type} onValueChange={setType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="student">Student</SelectItem><SelectItem value="staff">Staff</SelectItem><SelectItem value="parent">Parent</SelectItem></SelectContent></Select></div><div><Label className="text-xs" htmlFor="idNum">ID #</Label><Input id="idNum" name="idNum" value={idNum} onChange={(e) => setIdNum(e.target.value)} /></div></div>
            <div><Label className="text-xs" htmlFor="idName">Name</Label><Input id="idName" name="idName" value={name} onChange={(e) => setName(e.target.value)} /></div>
            <div><Label className="text-xs" htmlFor="idTemplate">Template</Label><Select name="idTemplate" value={templateId} onValueChange={setTemplateId}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{templates.map((t) => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent></Select></div>
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs" htmlFor="issueDate">Issue Date</Label><Input id="issueDate" name="issueDate" type="date" value={issue} onChange={(e) => setIssue(e.target.value)} /></div><div><Label className="text-xs" htmlFor="expiryDate">Expiry</Label><Input id="expiryDate" name="expiryDate" type="date" value={expiry} onChange={(e) => setExpiry(e.target.value)} /></div></div>
          </div>
      ...
      <Dialog open={tplOpen} onOpenChange={setTplOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4">
            <div><Label className="text-xs" htmlFor="tplName">Name</Label><Input id="tplName" name="tplName" value={tplName} onChange={(e) => setTplName(e.target.value)} /></div>
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs" htmlFor="bgColor">Bg Color</Label><Input id="bgColor" name="bgColor" type="color" value={bgColor} onChange={(e) => setBgColor(e.target.value)} /></div><div><Label className="text-xs" htmlFor="textColor">Text Color</Label><Input id="textColor" name="textColor" type="color" value={textColor} onChange={(e) => setTextColor(e.target.value)} /></div></div>
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs" htmlFor="font">Font</Label><Select name="font" value={font} onValueChange={setFont}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{FONTS.map((f) => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent></Select></div><div><Label className="text-xs" htmlFor="logoUrl">Logo URL</Label><Input id="logoUrl" name="logoUrl" value={logoUrl} onChange={(e) => setLogoUrl(e.target.value)} /></div></div>
      ...
      <Dialog open={bpOpen} onOpenChange={setBpOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs" htmlFor="bpStudent">Student</Label><Input id="bpStudent" name="bpStudent" value={bpStudent} onChange={(e) => setBpStudent(e.target.value)} /></div><div><Label className="text-xs" htmlFor="bpRoute">Route</Label><Input id="bpRoute" name="bpRoute" value={bpRoute} onChange={(e) => setBpRoute(e.target.value)} /></div></div>
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs" htmlFor="bpPickup">Pickup</Label><Input id="bpPickup" name="bpPickup" value={bpPickup} onChange={(e) => setBpPickup(e.target.value)} /></div><div><Label className="text-xs" htmlFor="bpDropoff">Dropoff</Label><Input id="bpDropoff" name="bpDropoff" value={bpDropoff} onChange={(e) => setBpDropoff(e.target.value)} /></div></div>
            <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs" htmlFor="bpFee">Fee (₹)</Label><Input id="bpFee" name="bpFee" type="number" value={bpFee} onChange={(e) => setBpFee(e.target.value)} /></div><div><Label className="text-xs" htmlFor="bpExpiry">Expiry</Label><Input id="bpExpiry" name="bpExpiry" type="date" value={expiry} onChange={(e) => setExpiry(e.target.value)} /></div></div>
          </div>
