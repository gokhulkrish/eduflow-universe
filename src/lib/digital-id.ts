...
export function createTemplate(t: Omit<IDTemplate, "id">): IDTemplate {
  const items = getTemplates(); const n = { ...t, id: crypto.randomUUID() };
...
export function createID(e: Omit<DigitalIDEntry, "id" | "created_at" | "qr_data">): DigitalIDEntry {
  const items = getIDs();
  const n: DigitalIDEntry = { ...e, id: crypto.randomUUID(), qr_data: `ID:${e.id_number}|Type:${e.entity_type}|Name:${e.entity_name}`, created_at: new Date().toISOString() };
...
export function createBusPass(b: Omit<BusPass, "id">): BusPass {
  const items = getBusPasses(); const n = { ...b, id: crypto.randomUUID() };
