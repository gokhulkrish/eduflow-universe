...
const fieldGroups: ImportModuleFieldGroup[] = [
  {
    ...
    fields: [
      { key: "name", label: "Device Name", aliases: ["name", "device name", "sensor name", "equipment"], required: true },
      { key: "type", label: "Device Type", aliases: ["type", "device_type", "device type", "sensor type", "category"], required: true },
  ...
  },
  {
    ...
    fields: [
      { key: "ip_address", label: "IP Address", aliases: ["ip_address", "ip", "ip address"] },
      { key: "location", label: "Location", aliases: ["location", "building", "room", "site"] },
      { key: "protocol", label: "Protocol", aliases: ["protocol", "comm protocol", "interface", "connection"] },
    ],
  },
  {
    title: "Status",
...
async function loadExistingRecords(): Promise<Record<string, unknown>[]> {
  const { data } = await (supabase.from("iot_devices") as any).select("id, name, device_type, brand, model, ip_address, location, status, protocol").order("name");
  return (data || []).map((r: Record<string, unknown>) => ({
    id: r.id, name: r.name, type: r.device_type, brand: r.brand, model: r.model,
    ip_address: r.ip_address, location: r.location, status: r.status, protocol: r.protocol,
  }));
}
...
async function commitRows(rows: ImportPreviewRow[], _batch: ImportBatch, signal?: AbortSignal): Promise<ImportCommitResult> {
  ...
  async function processRow(row: ImportPreviewRow): Promise<void> {
    ...
    try {
      ...
      db.protocol = row.mapped.protocol ?? row.sourceRow.protocol ?? null;
      const { data: existing } = await supabase.from("iot_devices").select("id").eq("name", name).maybeSingle();
      if (row.action === "insert" && !existing) {
        const { data: res, error } = await (supabase.from("iot_devices") as any).insert(db).select().single();
        ...
        const { error } = await (supabase.from("iot_devices") as any).update(db).eq("id", targetId);
        if (error) { result.failed++; result.errors.push({ rowNumber: row.sourceRowIndex, message: error.message }); }
  ...
  emitAppSync("sms.iot_devices.v1");
  return result;
}
...
async function rollbackRows(rollbackData: ImportRollbackEntry[]): Promise<{ success: boolean; restored: number }> {
  ...
  for (const entry of rollbackData) {
    try {
      if (entry.changeType === "inserted") { const { error } = await supabase.from("iot_devices").delete().eq("id", entry.studentKey); if (error) throw error; restored++; }
      else if (entry.changeType === "updated") { const { error } = await (supabase.from("iot_devices") as any).update(entry.previousState).eq("id", entry.studentKey); if (error) throw error; restored++; }
    } catch { success = false; }
  }
  return { success, restored };
}

export const iotDevicesModule: ImportModule = {
  id: "iot-devices", name: "IoT Devices",
  description: "Import IoT device records with type, brand, model, IP, and location",
  icon: "Cpu", fieldGroups, matchStrategies,
